  ...
  <div class="p-2 bg-slate-100 rounded-lg text-black mt-10">
    <form action="" method="post" novalidate>
      ...
      <div class="mb-4">
        ...
        <input type="text" id="game" name="game" class="w-full p-2 border border-slate-300 rounded" value="<?= isset($game) ? htmlspecialchars($game['game']) : '' ?>">
      </div>
      <div class="mb-4">
        <label for="genre" class="block text-sm font-medium mb-1">Genre:</label>
        <input type="text" id="genre" name="genre" class="w-full p-2 border border-slate-300 rounded" value="<?= isset($game) ? htmlspecialchars($game['genre']) : '' ?>">
      </div>
      <div class="mb-4">
        <label for="description" class="block text-sm font-medium mb-1">Beschreibung:</label>
        <input type="text" id="description" name="description" class="w-full p-2 border border-slate-300 rounded" value="<?= isset($game) ? htmlspecialchars($game['description']) : '' ?>">
      </div>
      <input type="submit" value="Edit Game" class="py-1 px-3 bg-blue-500 text-white rounded hover:bg-blue-300 hover:text-black">
    </form>
