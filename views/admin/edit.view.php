  ...
  <div class="p-2 bg-slate-100 rounded-lg text-black mt-10">
    <form action="" method="post" novalidate>
      <input type="hidden" name="id"
        value="<?= isset($game) && $game instanceof Game ? htmlspecialchars($game->getId()) : '' ?>">
      <div class="mb-4">
        <label for="name" class="block text-sm font-medium mb-1">Name:</label>
        <input type="text" id="name" name="name" class="w-full p-2 border border-slate-300 rounded"
          value="<?= isset($game) && $game instanceof Game ? htmlspecialchars($game->getName()) : '' ?>">
      </div>
      <div class="mb-4">
        ...
        <input type="text" id="genre" name="genre" class="w-full p-2 border border-slate-300 rounded"
          value="<?= isset($game) && $game instanceof Game ? htmlspecialchars($game->getGenre()) : '' ?>">
      </div>
      <div class="mb-4">
        ...
        <input type="text" id="description" name="description" class="w-full p-2 border border-slate-300 rounded"
          value="<?= isset($game) && $game instanceof Game ? htmlspecialchars($game->getDescription()) : '' ?>">
      </div>
      <input type="submit" value="Edit Game"
        class="py-1 px-3 bg-blue-500 text-white rounded hover:bg-blue-300 hover:text-black">
    </form>
